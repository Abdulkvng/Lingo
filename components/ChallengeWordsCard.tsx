...
const CrossIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-lingo-text-secondary/30 flex-shrink-0"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
);
...
const ChallengeWordsCard: React.FC<ChallengeWordsCardProps> = ({ words }) => {
    ...
    return (
        <div className="bg-lingo-card-bg p-4 rounded-2xl border border-border-gray mb-4">
            <h3 className="font-bold text-lg text-lingo-text mb-3">Challenge Words</h3>
            <ul className="space-y-3">
                {words.map(({ word, used, feedback }) => (
                    <li key={word} className="flex items-start space-x-3">
                        {used ? <CheckIcon /> : <CrossIcon />}
                        <div>
                            <p className="text-lingo-text font-semibold">{word}</p>
                            <p className="text-sm text-lingo-text-secondary">{feedback}</p>
                        </div>
                    </li>
                ))}
            </ul>
