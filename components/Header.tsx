...
const ProfileIcon: React.FC<{onClick?: ()=>void}> = ({onClick}) => (
    <button onClick={onClick} className="text-lingo-text-secondary hover:text-lingo-blue transition-colors">
  ...
  return (
    <div className="flex items-center justify-between p-4 border-b border-border-gray relative h-16 flex-shrink-0">
      <div className="absolute left-4">
        {onBack && (
          <button onClick={onBack} className="text-lingo-text-secondary hover:text-lingo-blue transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6"/></svg>
          </button>
        )}
      </div>

      <h1 className="text-lg font-bold text-lingo-text text-center w-full">{title}</h1>
      
      <div className="absolute right-4 flex items-center space-x-4">
        {showProfileIcon && <ProfileIcon onClick={onProfileClick} />}
        {!showProfileIcon && <div className="text-lg font-bold text-lingo-blue tracking-wider">LINGO</div>}
      </div>
    </div>
  );
};
